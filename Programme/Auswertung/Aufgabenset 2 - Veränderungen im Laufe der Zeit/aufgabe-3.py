...
temp1_zm = np.mean(temp_zm[date1_index:date1_index+119,:,:], axis=0)
...
title='Temperature [K] '+date1 
title_file='jahres_plots/temp_'+date1[0:4]+'.png'

steps=10

plotten(temp1_zm,min_T,max_T,title,title_file,steps)

# b) 2. Zeitpunkt
for q in range(1, 13):
  date2 = str(1970 + 10*q) + "01-" + str(1979 + 10*q) + "12"
  date2_index = 240 + 120*q - 1
  print('date2: ',times[date2_index])
  temp2_zm = np.mean(temp_zm[date2_index:date2_index+119,:,:], axis=0)
  title='Temperature '+date2
  title_file='jahrzehnte_plots/temp_'+date2+'.png'
  steps=10

  plotten(temp2_zm,min_T,max_T,title,title_file,steps)
  ...
  title_file='differenzen_jahrzehnte/temp_'+date2+'-'+date1+'.png'

  plotten(temp_diff,min_diff_T,max_diff_T,title,title_file, steps)
